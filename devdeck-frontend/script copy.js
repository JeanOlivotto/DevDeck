...
async function signup(email, password, name) {
    return await fetchApi('/auth/signup', { method: 'POST', body: JSON.stringify({ email, password, name }) }, true);
}
    ...
    try {
        ...
        if (payload.whatsappNumber) {
            ...
            if (!whatsappRegex.test(payload.whatsappNumber)) {
                showAlert('Formato WhatsApp inválido. Use: +XXXXXXXXXXX (com DDI)');
                updateToggleUI();
                return;
            }
        }
...
async function createBoard(name) {
    ...
    try {
        ...
        if (error.message?.includes('já existe')) {
            ...
            showAlert(error.message, 'Erro ao Criar Quadro');
        }
        throw error;
    }
}
...
async function updateBoard(boardId, boardData) {
    ...
    try {
        ...
        if (error.message?.includes('já existe')) {
            ...
            showAlert(error.message, 'Erro ao Renomear');
        }
        throw error;
    }
}
...
function connectWebSocket() {
    if (!authToken) {
        console.warn('Sem token, WebSocket não conectado.');
        updateWhatsappUI('Desconectado (sem token)');
        return;
    }
    ...
    socket.on('connect', () => {
        ...
        updateWhatsappUI('close');
    });

    socket.on('disconnect', (reason) => {
        console.warn('WebSocket Desconectado:', reason);
        updateWhatsappUI('Desconectado (Socket)');
    });
    ...
    socket.on('whatsapp_qr_code', (qrDataUrl) => {
        ...
        if (whatsappDisconnectBtn) whatsappDisconnectBtn.classList.remove('hidden');
    });
...
function updateWhatsappUI(status) {
    ...
    switch (status) {
        ...
        case 'error':
            ...
            break;
        default:
            if (whatsappStatusText) whatsappStatusText.textContent = `Status: ${status}`;
            if (whatsappConnectBtn) whatsappConnectBtn.classList.remove('hidden');
...
function renderBoardSelectors(boards) {
    ...
    newBtn.addEventListener('click', () => openBoardModal());
    boardsContainer.appendChild(newBtn);
}

function setActiveBoardUI(id) {
    document.querySelectorAll('.board-button').forEach(b => b.classList.toggle('active-board', parseInt(b.dataset.boardId, 10) === id));
}
...
function createTaskElement(task) {
    ...
    d.className = 'task-card bg-[#2e335a] rounded-lg shadow-md p-3 mb-3 cursor-grab text-white';
    d.draggable = true;
    d.dataset.taskId = task.id;
    d.dataset.status = task.status;
    const t = document.createElement('strong');
    t.className = 'block font-semibold mb-1 truncate';
    t.textContent = task.title;
    d.appendChild(t);
    if (task.description) {
        const s = document.createElement('small');
        s.className = 'block text-gray-400 text-sm break-words';
        s.textContent = task.description;
        d.appendChild(s);
    }
...
function openTaskModal(t = null) {
    ...
    if (t) {
        ...
        taskStatusSelect.value = t.status;
        ...
        taskStatusSelect.value = (btn?.dataset.status) || 'TODO';
    }
...
async function handleTaskFormSubmit(e) {
    ...
    if (!d.boardId) {
        showAlert('Quadro não selecionado.');
        return;
    }
    if (!d.title) {
        showAlert('O título da tarefa é obrigatório.');
        return;
    }
    try {
        if (id) await updateTask(id, d);
        else await createTask(d);
        closeTaskModal();
        loadTasksForBoard(currentBoardId);
    } catch (err) {
        console.error('Falha salvar tarefa:', err);
    }
}
...
async function handleDeleteTask() {
    ...
    if (confirmed) {
        try {
            ...
            console.error('Falha excluir tarefa:', err);
        }
    }
}
...
function openBoardModal(b = null) {
    ...
    if (b) {
        ...
        boardModalSaveButton.textContent = 'Salvar Alterações';
    } else {
        boardModalTitle.textContent = 'Novo Quadro';
        boardModalSaveButton.textContent = 'Criar Quadro';
    }
...
async function handleBoardFormSubmit(e) {
    e.preventDefault();
    const n = boardNameInput.value.trim();
    const id = boardIdInput.value;
    if (!n) return;
    try {
        let board;
        if (id) board = await updateBoard(id, { name: n });
        else board = await createBoard(n);
        closeBoardModal();
        await loadInitialData(board.id);
    } catch (err) { }
}
...
async function handleDeleteBoard(id, name) {
    ...
    if (confirmed) {
        try {
            ...
            console.log(`Quadro ${id} excluído.`);
            const wasCurrent = currentBoardId === id;
            await loadInitialData(wasCurrent ? null : currentBoardId);
        } catch (err) {
            console.error(`Falha excluir quadro ${id}:`, err);
        }
    }
}
    ...
    if (newStatus !== oldStatus) {
        try {
            ...
            if (originalColumnTasks) {
                originalColumnTasks.appendChild(locallyDraggedElement);
...
function setupDragAndDrop() {
    document.querySelectorAll('.task-card').forEach(t => {
        t.removeEventListener('dragstart', handleDragStart);
        t.removeEventListener('dragend', handleDragEnd);
    });
    document.querySelectorAll('.column .tasks').forEach(c => {
        c.removeEventListener('dragover', handleDragOver);
        c.removeEventListener('dragenter', handleDragEnter);
        c.removeEventListener('dragleave', handleDragLeave);
...
async function loadTasksForBoard(id) {
    ...
    try {
        ...
        console.error(`Erro carregar tasks ${id}:`, err);
        renderTasks([]);
    }
}
...
async function loadInitialData(selectId = null) {
    ...
    try {
        ...
        if (allBoards.length === 0) {
            ...
            if (!initId) {
                ...
                for (const n of defaults) {
                    const b = allBoards.find(b => b.name === n);
                    if (b) {
                        initId = b.id;
                        found = true;
                        break;
        ...
        loadingIndicator.classList.add('hidden');
        if (authToken) {
            boardsContainer.innerHTML = '<span class="text-red-500 p-3">Erro ao carregar</span>';
            kanbanBoardSection.classList.add('hidden');
            noBoardsMessage.classList.add('hidden');
            renderTasks([]);
        }
...
whatsappConnectBtn.addEventListener('click', () => {
  if (!socket) {
    showAlert('WebSocket não conectado. Reconectando...');
    connectWebSocket();
    // Aguardar conexão da socket
    setTimeout(() => {
      if (socket && socket.connected) {
    ...
    showAlert('WebSocket desconectado. Tentando reconectar...');
    connectWebSocket();
  }
});
...
whatsappConnectBtn.addEventListener('click', () => {
    if (socket && socket.connected) {
        if (whatsappStatusText) whatsappStatusText.textContent = 'Status: Solicitando conexão...';
        socket.emit('request_whatsapp_connect');
    } else {
...
whatsappDisconnectBtn.addEventListener('click', () => {
    if (socket && socket.connected) {
        if (whatsappStatusText) whatsappStatusText.textContent = 'Status: Desconectando...';
...
toggleWhatsApp.addEventListener('change', (e) => {
    ...
    updateUserSettings({ notifyViaWhatsApp: wantsWhatsApp });
});
...
whatsappNumberInput.addEventListener('change', (e) => {
    ...
    if (currentUserSettings.whatsappNumber !== newNumber) {
        currentUserSettings.whatsappNumber = newNumber || null;
...
loginForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    loginError.classList.add('hidden');
    const em = loginEmailInput.value;
...
signupForm.addEventListener('submit', async (e) => {
    ...
    const cpw = signupConfirmPasswordInput.value;
    if (!name) {
        signupError.textContent = 'O nome é obrigatório.';
    ...
    try {
        ...
        signupError.textContent = err.message || 'Erro cadastro.';
        signupError.classList.remove('hidden');
    }
});
...
document.body.addEventListener('click', function (event) {
    if (event.target.matches('.add-task-button')) {
        if (!currentBoardId) {
            showAlert('Por favor, selecione ou crie um quadro primeiro.');
            return;
        }
        openTaskModal();
        const s = event.target.dataset.status || 'TODO';
        taskStatusSelect.value = s;
    }
});
